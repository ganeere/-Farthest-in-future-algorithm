...
int main(int argc, char** argv) {
	...
	while(i<element_number){
		...
		if (i == 0) { // if first
			n->prev = NULL;
			n->next = NULL;
		} else if (i == element_number - 1) { // for last
			n->prev = addresses[i - 1];
			n->next = NULL;
			addresses[i - 1]->next = n;
		} else {
			n->prev = addresses[i - 1]; //arrangment for middle
			n->next = NULL;
			addresses[i - 1]->next = n;
		}
		i++;
		...
		for (int i = request_number - 1; i >= 0; i--) {
			...
			if (n->prev == NULL) {
				fif[i] = addresses[element_number - 1]->element;
			} else {
				fif[i] = n->prev->element;
			}
		}
		...
		// cout << "cache hit" << endl;
		int j=0;
	while(j< request_number){

		if (cache[requests[j]] == 0) {
			cout << "cache miss" << endl;
			if (cache_size == capacity) {
				cache[fif[j]] = 0;
				cache[requests[j]] = 1;
				cout << "cache is full, element " << fif[j] << " is evicted" << endl;
				cout << "element " << requests[j] << " is added into the cache" << endl;
			} else {
				cache[requests[j]] = 1;
				cache_size++;
				cout << "element " << requests[j] << " is added into the cache" << endl;
